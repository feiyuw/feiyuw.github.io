...
At this moment, I wrote some test robot cases to verify the script, the developing was TDD, and agile. In that two days, the first workable version was finished, including most of the checkpoints. The code is at below:
...
class LintBuilder(object):
    ...
    def build(self):
        ...
        self._checkers = sorted(_checkers, key=lambda x: x._id)
        return self

    def lint(self):
        ''' do the static analysis '''
        for _checker in self._checkers:
            self._suite.visit(_checker)
